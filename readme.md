...
进入任意星球页后，点击右下角的 `HAND CTRL`，允许摄像头权限即可启用手势控制：移动手掌旋转视角，张开或捏合拇指与食指可放大、缩小星球。缩放会同步改变主表面粒子的密度、亮度与尺寸；进入最后四分之一的近景区间后，粒子会叠加高频无规则游走并径向爆散，在离开屏幕前打破原有轨道。所有逐粒子效果都由 GPU 完成。识别模型已随项目保存在本地，视频帧仅在浏览器内处理，不会上传。

摄像头 API 需要安全上下文。若直接打开 `file://` 页面时浏览器拒绝摄像头，请在项目目录运行 `python3 -m http.server 8000`，再访问 `http://localhost:8000/earth.html`。关闭手势控制或离开页面后，摄像头会立即释放。
